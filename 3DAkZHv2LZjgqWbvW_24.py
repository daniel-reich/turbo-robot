...
![Graph Example](https://edabit-
challenges.s3.amazonaws.com/adjacent_nodes_1+.png)

There are two types of graphs; directed and undirected. In an undirected
graph, the edges between nodes have no particular direction (like a two-way
street) whereas in a directed graph, each edge has a direction associated with
it (like a one-way street).
...
We can encode graphs using an adjacency matrix. An adjacency matrix for a
graph with "n" nodes is an "n * n" matrix where the entry at row "i" and
column "j" is a 0 if nodes "i" and "j" are not adjacent, and 1 if nodes "i"
and "j" are adjacent.
...
![Graph Example](https://edabit-
challenges.s3.amazonaws.com/adjacent_nodes_2.png)
...
![Graph Example 2](https://edabit-
challenges.s3.amazonaws.com/adjacent_nodes_3.png)
...
def is_adjacent(matrix, node_source, node_target):
  return matrix[node_source][node_target] == 1
