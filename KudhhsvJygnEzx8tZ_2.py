...
def pk_special_winner(pk1, pk2):
    if (pk1, pk2) in [(9, 62), (72, 99), (80, 98), (79, 96)]:
        return -1
    types1, types2 = [], []
    for d in pk_data:
        if d["Number"] == pk1:
            types1.append(d["Type1"])
            types1.append(d["Type2"])
        elif d["Number"] == pk2:
            types2.append(d["Type1"])
            types2.append(d["Type2"])
        if types1 and types2:
            break
​
    ef1_1 = type_effectiveness[types1[0]][types2[0]]
    if types2[1]:
        ef1_1 *= type_effectiveness[types1[0]][types2[1]]
    if types1[1]:
        ef1_2 = type_effectiveness[types1[1]][types2[0]]
        if types2[1]:
            ef1_2 *= type_effectiveness[types1[1]][types2[1]]
        ef1 = (ef1_1 + ef1_2) / 2
    else:
        ef1 = ef1_1
​
    ef2_1 = type_effectiveness[types2[0]][types1[0]]
    if types1[1]:
        ef2_1 *= type_effectiveness[types2[0]][types1[1]]
    if types2[1]:
        ef2_2 = type_effectiveness[types2[1]][types1[0]]
        if types1[1]:
            ef2_2 *= type_effectiveness[types2[1]][types1[1]]
        ef2 = (ef2_1 + ef2_2) / 2
    else:
        ef2 = ef2_1
    ef1, ef2 = round(ef1, 3), round(ef2, 3)
    return pk1 if ef1 > ef2 else pk2 if ef2 > ef1 else -1
