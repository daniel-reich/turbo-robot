...
Functions that consists only of a return, can be written as oneliner with a
lambda function.
...
`lambda a, b: a == b` is technically an anonymous function. However, it can be
assigned to the identifier `are_same` and it can then be called using
`are_same()`.
...
for i in{2,3,5,7,11}:exec("add_{}=lambda n:n+{}".format(i,i))
