...
In this challenge, you have to establish if a given integer is an Astonishing
number. An Astonishing number is an integer that, when partitioned into two
parts `a` (left) and `b` (right), is equal to the sum of the consecutive
numbers from `a` up to `b` (if `a` is lower than `b`), or from `b` up to `a`
